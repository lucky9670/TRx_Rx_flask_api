...
class   cognito():
    def __init__(self):
        ...
        self.userpool_id = "us-east-2_aKGAa2REZ"
        self.app_client_id = '5tl90es0tnga6408edb99mcgmq'
        self.keys_url = 'https://cognito-idp.{}.amazonaws.com/{}/.well-known/jwks.json'.format(self.region,self.userpool_id)
        self.url_get_tokens = 'https://mechanic-organization.auth.us-east-2.amazoncognito.com/oauth2/token'
    ...
    def add_org_id(self,data,org_id):
        err,client = self.get_client()
        if(err):
            return err,{}
        try:
            print("From Cognito function printing org_id")
            print(org_id)     
            response = client.admin_update_user_attributes(
                UserPoolId=self.userpool_id,
                Username= data['email'],

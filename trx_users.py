...
class trx_user():
    ...
    def verify_data_all(self):
        ...
        if len(self.f_name) == 0 or len(self.f_name)  >= 65:
            err_trx['f_name'].append('First name cannot be empty or more than 64 words')
        
        
        
        #verify l_name:
        if len(self.l_name) == 0 or len(self.l_name) >= 65:
            err_trx['l_name'].append('Last name cannot be empty or more than 64 words')

        #verify roles:
        print("From verfying roles")
        print(self.roles)
        if self.roles != '4' and self.roles != '8':
            err_trx['roles'].append('roles can be only 4,8')



        #verify email:
        if len(self.email) == 0 or len(self.email)  >= 65:
            err_trx['email'].append('Email cannot be empty or more than 64 words')
...
#       if not num.isdigit():
#            err_trx['contact_num'].append('Contact number can only be in numbers')
        
#        if len(code) > 3 or  len(code) == 0 or code[0] != "+":
#            err_trx['contact_num'].append("Country codes should be between 1-4 letters and should begin with '+' sign")
#        else:
#            if err_trx['country'] == []:
#                print(code,country_codes[self.country])
#                if country_codes[self.country] != code:
#                    err_trx['contact_num'].append("Wrong country code for {}. Correct is '{}'".format(self.country,country_codes[self.country]))


#        if len(num) != 10:
#            err_trx['contact_num'].append('Contact number should be 10 digits without the country code')
